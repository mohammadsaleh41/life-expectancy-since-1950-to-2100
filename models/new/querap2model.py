...
Original file is located at
    https://colab.research.google.com/drive/1qiPxXJZGTbZyuvfi0IYG3SZxJf1SM2vn
"""
...
model_data = pd.read_csv('/content/ModelData.csv').drop(columns=['Unnamed: 0'])
estimate_data = pd.read_csv('/content/ModelEstimationData.csv')
countries = list(pd.read_csv('/content/CountryNames.csv').iloc[:, 0])
...
model.add(K.layers.Dense(units=20, input_dim=44, kernel_initializer=init,
                         activation='tanh'))
model.add(K.layers.Dense(units=10, activation='tanh')) 
model.add(K.layers.Dense(units=10, activation='tanh'))
...
h = model.fit(train_X, train_Y, epochs=max_epochs, verbose=1)
...
pred_df = pd.read_csv('/content/ModelEstimationData.csv').drop(columns=['Region, subregion, country or area *',
                                                                        'ISO3 Alpha-code'])
